...
import ConditionConstructor from '../src/condition'
...
const conditionConstructor = new ConditionConstructor()

describe('Condition', () => {
  describe('constructor', () => {
    it('fact conditions have properties', () => {
      const properties = condition()
      const subject = conditionConstructor.construct(properties.all[0])
    ...
    it('boolean conditions have properties', () => {
      const properties = condition()
      const subject = conditionConstructor.construct(properties)
      expect(subject).to.have.property('operator')
  ...
  describe('toJSON', () => {
    it('converts the condition into a json string', () => {
      ...
      const condition = conditionConstructor.construct(properties)
  ...
  describe('evaluate', () => {
    ...
    function setup (options, factValue) {
      ...
      condition = conditionConstructor.construct(properties)
    ...
    it('evaluates "equal"', async () => {
      ...
      expect((await condition.evaluate(almanac, operators, conditions)).result).to.equal(true)
      setup({ operator: 'equal' }, 5)
      expect((await condition.evaluate(almanac, operators, conditions)).result).to.equal(false)
    })

    it('evaluates "equal" to check for undefined', async () => {
      condition = conditionConstructor.construct({ fact: 'age', operator: 'equal', value: undefined })
  ...
  describe('objects', () => {
    describe('.path', () => {
      it('extracts the object property values using its "path" property', async () => {
        const condition = conditionConstructor.construct({ operator: 'equal', path: '$.[0].id', fact: 'age', value: 50 })
    ...
    describe('jsonPath', () => {
      it('allows json path to extract values from complex facts', async () => {
        const condition = conditionConstructor.construct({ operator: 'contains', path: '$.phoneNumbers[*].type', fact: 'users', value: 'iPhone' })
        const userData = {
          phoneNumbers: [
            {
              type: 'iPhone',
  ...
  describe('boolean operators', () => {
    it('throws if not not an array', () => {
      ...
      expect(() => conditionConstructor.construct(conditions)).to.throw(/"all" must be an array/)
    })
    ...
    it('throws if is an array and condition is "not"', () => {
      ...
      expect(() => conditionConstructor.construct(conditions)).to.throw(/"not" cannot be an array/)
    })
  ...
  describe('atomic facts', () => {
    ...
    it('throws for a missing "operator"', () => {
      ...
      expect(() => conditionConstructor.construct(conditions)).to.throw(/Condition: constructor "operator" property required/)
    })

    it('throws for a missing "fact"', () => {
      const conditions = condition()
      delete conditions.all[0].fact
      expect(() => conditionConstructor.construct(conditions)).to.throw(/Condition: constructor "fact" property required/)
    })

    it('throws for a missing "value"', () => {
      const conditions = condition()
      delete conditions.all[0].value
      expect(() => conditionConstructor.construct(conditions)).to.throw(/Condition: constructor "value" property required/)
    })
  })
