...
describe('Condition', () => {
  ...
  describe('evaluate', () => {
    ...
    context('validations', () => {
      ...
      it('throws when missing an almanac', () => {
        return expect(
          condition.evaluate(undefined, operators)
        ).to.be.rejectedWith('almanac required');
      });
      it('throws when missing operators', () => {
        return expect(
          condition.evaluate(almanac, undefined)
        ).to.be.rejectedWith('operatorMap required');
      });
      it('throws when run against a boolean operator', () => {
        condition.all = [];
        return expect(
          condition.evaluate(almanac, operators)
        ).to.be.rejectedWith('Cannot evaluate() a boolean condition');
      });
    });
    ...
    it('evaluates "equal"', async () => {
      ...
      expect(
        (await condition.evaluate(almanac, operators, 50)).result
      ).to.equal(true);
      setup({ operator: 'equal' }, 5);
      expect((await condition.evaluate(almanac, operators, 5)).result).to.equal(
        false
      );
    });
    ...
    it('evaluates "equal" to check for undefined', async () => {
      ...
      expect((await condition.evaluate(almanac, operators)).result).to.equal(
        true
      );
  ...
  describe('boolean operators', () => {
    it('throws if not not an array', () => {
      ...
      expect(() => new Condition(conditions)).to.throw(
